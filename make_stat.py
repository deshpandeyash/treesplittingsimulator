...
def plot_conf_interval(interval_array, theoretical_mean, save_fig=False, today_folder=""):
    ...
    figname = F"{today_folder}cof_interval_plot"
    if save_fig:
        plt.savefig(F"{figname}.png", dpi=300)
    plt.show()


def make_histogram_discrete(data, sim, setting, xlabel='Not Given', save_fig=False, today_folder=""):
    data = np.array(data)
    ...
    plt.legend()
    plt.xlabel(xlabel)
    figname = F"{today_folder}{xlabel}{setting.statictreewindow.users}K{sim.sim_param.K}Q{sim.sim_param.SPLIT}" \
              F"histogram_static"
    if save_fig:
        plt.savefig(F"{figname}.png", dpi=300)
    plt.show()
...
def make_histogram_cont(data, sim, xlabel='Not Given', conf_ints=None, theoretical_mean=None, save_fig=False,
    ...
    plt.xlabel(xlabel)
    plt.legend()
    figname = F"{today_folder}{xlabel}K{sim.sim_param.K}Q{sim.sim_param.SPLIT}histogram_static"
    if save_fig:
